	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("metafauna")) {
			if (!MetafaunaModPlayerAnimationAPI.animations.isEmpty())
				MetafaunaModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel model = (PlayerModel) (Object) this;
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			playingAnimation = "";
			MetafaunaModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		MetafaunaModPlayerAnimationAPI.PlayerAnimation animation = MetafaunaModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = MetafaunaModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				MetafaunaMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			MetafaunaModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, MetafaunaModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			MetafaunaModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
